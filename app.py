...
st.set_page_config(page_title="Meu Coach Financeiro", page_icon="💰", layout="wide")

# --- MEMÓRIA DO SISTEMA (SESSION STATE) ---
if 'historico' not in st.session_state:
    st.session_state.historico = []

# --- ESTILO CSS (CORREÇÃO PARA MODO ESCURO) ---
st.markdown("""
    <style>
...
# Campos de entrada simplificados para não travar a digitação
ganho = st.sidebar.number_input("Ganho Mensal (R$)", min_value=0.0, value=0.0, step=100.0, format="%.2f")
fixas = st.sidebar.number_input("Despesas Fixas (R$)", min_value=0.0, value=0.0, step=10.0, format="%.2f")
bobeiras = st.sidebar.number_input("Bobeiras/Lazer (R$)", min_value=0.0, value=0.0, step=10.0, format="%.2f")
investimento = st.sidebar.number_input("Investimento (R$)", min_value=0.0, value=0.0, step=10.0, format="%.2f")

# Garantindo que os cálculos funcionem mesmo com valores zerados
...
with c_conselho:
    st.subheader("💡 Dica do Coach")
    
    frases_boas = [
        "🌟 Excelente! Que tal investir uma parte desse saldo positivo?",
        "🚀 Você está no caminho certo. Constância é a chave da riqueza!",
        "💰 Dinheiro guardado hoje é tranquilidade garantida amanhã."
    ]
    
    frases_alerta = [
        "⚠️ Cuidado com as bobeiras! Elas podem adiar seus sonhos.",
